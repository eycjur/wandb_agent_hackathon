...
2. 生成結果または任意の評価対象文を Judge LLM で評価
3. Score / Reason / 合格判定を表示
4. **人間フィードバック**: 評価対象の文章に対するあなたのスコア・コメントを送信（Judge プロンプト改善に活用）
...
- `sourceType`: 任意、`generated` / `generated_edited` / `manual`
...
| 人間フィードバック | ユーザーが評価対象の文章に付与するスコア・コメント |
